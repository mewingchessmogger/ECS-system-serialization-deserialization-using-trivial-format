...
namespace ECS{
    ...
    struct Pool : public IComponentPool{
        ...
        void assignComponentFields(Entity e, std::vector<Variable>&& vars) override{
           T comp{};
            std::vector<VariableAddress> addresses = comp.reflectAddress();
            ...
            for(int i{}; i < addresses.size(); i++){
                ...
                else if(std::holds_alternative<vec3*>(v.address)){
                    vec3* ptr = std::get<vec3*>(v.address);
                    *ptr = std::get<vec3>(vars[i].var);
                }else{
    ...
    class Registry{
        ...
        */
        ...
        void cleanResetAll(){
            for(auto& [sName, pool] : pools){
                pool->reset();
            }
            counter = 0;
            deadIDs.clear();
            deadIDs.shrink_to_fit();
            liveIDs.clear();
            liveIDs.shrink_to_fit();
            bool registryDirty = false;
        }
        ...
        void routeComponentToPool(Entity e, std::string_view sName, std::vector<Variable>&& vars){
